...
int is_str_xmas(char* str) {
    return (strcmp(str, "MAS") == 0) || (strcmp(str, "SAM") == 0);
}

int is_xmas(char* text, int n_lines, int line_len, int i, int j) {
    char diag[4];
    diag[3] = '\0';
    ...
    for (int a = 0; a < 3; ++a) {
        int new_i = i - 1 + a;
        int new_j = j - 1 + a;
        diag[a] = text[(new_i * line_len) + new_j];
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < n_lines - 1; ++i) {
        for (int j = 1; j < line_len - 1; ++j) {
            int x = is_xmas(text, n_lines, line_len, i, j);
            count += x;
