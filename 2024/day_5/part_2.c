...
ArrayList pages_after[100];  // WTF? Why can't I use max_page_num here? Its const.

int compare(const void *a, const void *b) {
    int page1 = *(int*) a;
    int page2 = *(int*) b;

    int page_2_after_1 = get_idx(&pages_after[page1], page2);
    if (page_2_after_1 != -1) {
        return -1;
    }
    return 1;
...
int main(int argc, char* argv[]) {
    ...
    while (cont) {
        ...
        if (!good_order) {
            qsort(pages.ptr, pages.len, sizeof(int), compare);        
            print(&pages);
